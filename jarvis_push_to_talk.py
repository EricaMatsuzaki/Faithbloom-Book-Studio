...
<div class="fb-jarvis-ui" id="fb-jarvis-ui">
  <img id="fb-jarvis-skin" class="fb-jarvis-skin" alt="FaithBloom Jarvis Premium Futurista">
...
export default function(component) {
  ...
  const skin=root.querySelector('#fb-jarvis-skin'),speech=root.querySelector('#fb-speech'),ptt=root.querySelector('#fb-ptt'),status=root.querySelector('#fb-ptt-status'),form=root.querySelector('#fb-text-form'),input=root.querySelector('#fb-text-input'),weather=root.querySelector('#fb-weather'),agenda=root.querySelector('#fb-agenda');
  const greeting=(data&&data.greeting)||'Olá, Erica! O que vamos fazer hoje?',reply=(data&&data.reply_text)||'',replyAudio=(data&&data.reply_audio)||'',replyToken=(data&&data.reply_token)||'',skinB64=(data&&data.skin_b64)||'',weatherStatus=(data&&data.weather_status)||'Pronto para consultar',agendaStatus=(data&&data.agenda_status)||'Ainda não conectada';
  if(skin&&skinB64&&!skin.getAttribute('src'))skin.setAttribute('src','data:image/jpeg;base64,'+skinB64);if(speech)speech.textContent=reply||greeting;if(weather)weather.textContent=weatherStatus;if(agenda)agenda.textContent=agendaStatus;
...
def push_to_talk(*,key: str="jarvis_push_to_talk",greeting: str="",reply_text: str="",reply_audio: str="",reply_token: str="",weather_status: str="Pronto para consultar",agenda_status: str="Ainda não conectada"):
    ...
    widget=factory("faithbloom_jarvis_premium_shell",html=HTML,css=_inline_css(),js=JS)
    return widget(key=key,data={"greeting":greeting,"reply_text":reply_text,"reply_audio":reply_audio,"reply_token":reply_token,"skin_b64":_skin_b64(),"weather_status":weather_status,"agenda_status":agenda_status},default={"recording":None,"submitted":None,"typed_request":None,"typed_submitted":None,"navigation":None,"navigation_submitted":None},on_recording_change=lambda:None,on_submitted_change=lambda:None,on_typed_request_change=lambda:None,on_typed_submitted_change=lambda:None,on_navigation_change=lambda:None,on_navigation_submitted_change=lambda:None)