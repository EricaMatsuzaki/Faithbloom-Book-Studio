...
export default function(component) {
  ...
  form?.addEventListener('submit',e=>{e.preventDefault();const text=String(input?.value||'').trim();if(!text)return;const id=`text-${Date.now()}`;setStatus('Pensando e executando…','thinking');setStateValue('typed_request',{id,text});if(input)input.value='';});
  root.querySelectorAll('[data-nav]').forEach(el=>el.addEventListener('click',()=>{const destination=el.getAttribute('data-nav');if(!destination)return;const id=`nav-${Date.now()}-${destination}`;setStateValue('navigation',{id,destination});}));return()=>{try{if(recorder&&recorder.state==='recording')recorder.stop();}catch(_){}cleanup();};
}
"""
...
def push_to_talk(*,key: str="jarvis_push_to_talk",greeting: str="",reply_text: str="",reply_audio: str="",reply_token: str="",weather_status: str="Pronto para consultar",agenda_status: str="Ainda não conectada"):
    ...
    return widget(
        key=key,
        data={"greeting":greeting,"reply_text":reply_text,"reply_audio":reply_audio,"reply_token":reply_token,"weather_status":weather_status,"agenda_status":agenda_status},
        default={"recording":None,"typed_request":None,"navigation":None},
        on_recording_change=lambda:None,
        on_typed_request_change=lambda:None,
