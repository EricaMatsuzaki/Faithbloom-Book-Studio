...
submit automatically. STT/TTS stay in jarvis_voice.py. This component owns the
browser interaction, greeting and visual speaking/listening states.
"""
...
<div class="jarvis-stage">
  ...
  <button id="ptt" type="button" aria-label="Segure o coração do Jarvis para falar">
    ...
    <div class="body"><div class="arm left"></div><div class="arm right"></div><div class="heart"><span>🎙️</span></div></div>
  </button>
...
#ptt{position:relative;z-index:3;width:270px;height:345px;border:0;background:transparent;padding:0;cursor:pointer;touch-action:none;user-select:none;-webkit-user-select:none;filter:drop-shadow(0 22px 35px rgba(0,0,0,.30));transition:transform .18s ease}#ptt:hover{transform:translateY(-3px)}#ptt.recording{transform:scale(1.03)}
...
.body{position:absolute;left:64px;top:201px;width:142px;height:111px;border-radius:36px 36px 46px 46px;background:linear-gradient(145deg,#fff,#d9e6f0 68%,#a9bbce);border:3px solid rgba(255,255,255,.92);box-shadow:inset -10px -10px 20px rgba(56,77,105,.14)}.arm{position:absolute;top:18px;width:32px;height:78px;border-radius:18px;background:linear-gradient(#eef7fd,#a9bfd1);border:2px solid rgba(255,255,255,.85)}.arm.left{left:-30px;transform:rotate(17deg)}.arm.right{right:-30px;transform:rotate(-17deg)}
...
@keyframes talk{from{height:7px;width:27px}to{height:26px;width:34px;border-radius:50%;border:5px solid #76f7f0}}@keyframes pulse{from{transform:translateX(-50%) scale(1)}to{transform:translateX(-50%) scale(1.09)}}@keyframes bloom{0%,100%{transform:rotate(-5deg) scale(1)}50%{transform:rotate(8deg) scale(1.08)}}@media(max-width:640px){.jarvis-stage{min-height:510px;padding:26px 10px 18px}.speech{font-size:.92rem;max-width:92%}#ptt{transform:scale(.9);margin:-10px 0}.name{margin-top:-18px}}
"""
...
export default function(component) {
  ...
  const speakNative=(text)=>{if(!('speechSynthesis'in window)||!text)return;try{window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang='pt-BR';u.rate=.98;u.pitch=1.03;u.onstart=()=>{stage.classList.add('speaking');setStatus('🔊 Jarvis está falando…');};u.onend=()=>{stage.classList.remove('speaking');setStatus('👆 Segure meu coração, fale e solte');};u.onerror=()=>stage.classList.remove('speaking');window.speechSynthesis.speak(u);}catch(_){stage.classList.remove('speaking');}};
  const speakReply=()=>{
    if(replyAudio){try{const a=new Audio('data:audio/mpeg;base64,'+replyAudio);a.onplay=()=>{stage.classList.add('speaking');setStatus('🔊 Jarvis está falando…');};a.onended=()=>{stage.classList.remove('speaking');setStatus('👆 Segure meu coração, fale e solte');};a.onerror=()=>speakNative(reply);const p=a.play();if(p&&p.catch)p.catch(()=>speakNative(reply));return;}catch(_){}}
    speakNative(reply);
  };
  try{
    if(reply&&replyToken&&sessionStorage.getItem('faithbloom_jarvis_reply')!==replyToken){sessionStorage.setItem('faithbloom_jarvis_reply',replyToken);setTimeout(speakReply,180);}
    else if(!reply&&!sessionStorage.getItem('faithbloom_jarvis_greeted')){sessionStorage.setItem('faithbloom_jarvis_greeted','1');setTimeout(()=>speakNative(greeting),400);}
  }catch(_){if(reply)setTimeout(speakReply,180);else setTimeout(()=>speakNative(greeting),400);}
