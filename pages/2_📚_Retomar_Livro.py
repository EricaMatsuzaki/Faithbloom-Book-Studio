...
aplicar_estilo()
hero("📚 Retomar um livro com roteiro pronto", "Já tem o roteiro? Pule direto pro Ilustrador.")
    ...
    for nome, p in s.get("personagens", {}).items():
        with st.expander(f"{nome} ({p.get('papel', '')})"):
            ...
            origem = st.radio(
                ...
                ["Deixar o agente criar", "Enviar minha própria imagem", "Transformar uma foto real (photo-to-cartoon)"],
                key=f"origem_{nome}",
            )
            if origem == "Enviar minha própria imagem":
                ...
                foto = st.file_uploader(
                    f"Foto real (sua, de familiar ou do pet) para virar {nome}",
                    type=["png", "jpg", "jpeg"], key=f"foto_{nome}",
                )
                detalhe = st.text_input("Algum detalhe extra a manter? (opcional)", key=f"detalhe_{nome}")
                if foto is not None and st.button(f"✨ Transformar foto em {nome}", key=f"transformar_{nome}"):
                    ...
                    with st.spinner("Transformando a foto no estilo da coleção..."):
                        personagem_gerado = gerar_personagem_a_partir_de_foto(
                            caminho_foto, nome, p.get("papel", "amigo"), gerar_imagem, detalhe,
                        )
                    s["personagens"][nome]["imagem_referencia"] = personagem_gerado["imagem_referencia"]
                    s["personagens"][nome]["origem_referencia"] = "gerada_a_partir_de_foto"
                    st.image(personagem_gerado["imagem_referencia"], width=250)

    st.markdown("### Ilustrações prontas (opcional)")
    st.caption("Se você já tem a arte de alguma cena pronta, envie aqui — o Ilustrador pula a "
               "geração dessas cenas específicas e usa sua imagem direto.")
    for cena in s.get("cenas_texto", []):
        arquivo_cena = st.file_uploader(
            f"Cena {cena['numero']} - {cena.get('texto', '')[:50]}...",
            type=["png", "jpg", "jpeg"], key=f"cena_pronta_{cena['numero']}",
        )
        if arquivo_cena is not None:
            caminho = os.path.join("saida_imagens", f"cena_enviada_{cena['numero']}.png")
            os.makedirs("saida_imagens", exist_ok=True)
            with open(caminho, "wb") as f:
                f.write(arquivo_cena.getbuffer())
            s.setdefault("imagens_cenas_enviadas", {})[cena["numero"]] = caminho
...
# ------------------------------------------------------------ PROCESSANDO
    ...
    progresso.progress(91, text="Pesquisando palavras-chave e categorias...")
    ...
    progresso.progress(99, text="Gerando material de lançamento...")
