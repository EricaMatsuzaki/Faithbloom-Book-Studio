...
aplicar_estilo()
hero("📖 Gerador de Livros Infantis - Erica Matsuzaki", "Escreva uma história nova do zero — do tema à capa pronta.")
...
# ----------------------------------------------------- REVISAR TEXTO
    ...
    for cena in s.get("cenas_texto", []):
        with st.expander(f"Cena {cena['numero']} — emoção: {cena.get('emocao', '')}"):
            ...
            st.caption("Já tem a ilustração dessa cena pronta? Envie aqui pra pular a geração por IA "
                       "só dela (as outras continuam sendo geradas normalmente).")
            arquivo_cena = st.file_uploader(
                "Ilustração pronta (opcional)", type=["png", "jpg", "jpeg"], key=f"cena_pronta_{cena['numero']}"
            )
            if arquivo_cena is not None:
                ...
                st.success(f"Cena {cena['numero']}: vai usar sua imagem, sem gerar por IA.")
