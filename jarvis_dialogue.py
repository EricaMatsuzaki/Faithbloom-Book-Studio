...
from openrouter_client import MODELO_TEXTO, OPENROUTER_BASE_URL, _json_resposta, _post_com_retry
...
Responda em no máximo 3 frases, salvo se a pessoa pedir detalhes.
...
def build_natural_reply(
    ...
    *,
    ...
    """Generate one natural reply while preserving FaithBloom guardrails."""
    ...
    req_id, request_sig, estimate, started = iniciar_requisicao("texto", MODELO_TEXTO, "jarvis-dialogue|" + signature)
    try:
        messages = [{"role": "system", "content": SYSTEM_PROMPT}]
        messages.extend(_history_messages(history))
        if context:
            messages.append({"role": "system", "content": "Contexto operacional confirmado pelo FaithBloom: " + json.dumps(context, ensure_ascii=False)})
        messages.append({"role": "user", "content": text})
        payload = {"model": MODELO_TEXTO, "messages": messages, "temperature": 0.45, "max_tokens": 220}
        response = _post_com_retry(f"{OPENROUTER_BASE_URL}/chat/completions", payload, 60)
        ...
        finalizar_requisicao(req_id, request_sig, "texto", MODELO_TEXTO, estimate, started, "sucesso", extrair_custo_reportado(data))
        return answer
    except Exception as exc:
        finalizar_requisicao(req_id, request_sig, "texto", MODELO_TEXTO, estimate, started, "erro", detalhe=sanitizar_texto(str(exc)))
        raise
